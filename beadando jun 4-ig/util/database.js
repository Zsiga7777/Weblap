...
db.prepare(`CREATE TABLE IF NOT EXISTS bills (id INTEGER PRIMARY KEY AUTOINCREMENT, sellerId INTEGER, buyerId INTEGER, billNumber STRING, created DATE, payDay DATE,deadline DATE, total INTEGER, afa INTEGER, FOREIGN KEY(sellerId) REFERENCES sellers(id) ON DELETE CASCADE, FOREIGN KEY(buyerId) REFERENCES buyers(id) ON DELETE CASCADE )`).run()
